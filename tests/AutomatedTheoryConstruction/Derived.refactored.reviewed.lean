...
theorem thm_godel_fixpoint_le_box_reft_top_000001 :
  ...
  intro g
  simpa using le_trans (ACR.gf_le_reft_top (g := g)) (ACR.reft_le_prov_reft (x := (⊤ : α)))
...
theorem thm_godel_fixpoint_le_box_self_000009 :
  ...
  intro g
  simpa using le_trans g.2.1 (ACR.reft_gf_le_box_gf (g := g))
...
theorem thm_henkin_fixpoint_le_box_reft_top_000002 :
  ...
  exact le_trans hle (ACR.prov_mono hle_reft_top)
...
theorem thm_godel_fixpoint_equiv_double_reft_top_000004 :
  ...
  exact ⟨le_trans hTop.1 hIdem.1, le_trans hIdem.2 hTop.2⟩
...
theorem main_thm_godel_fixpoint_unique_equiv :
  ...
  intro g₁ g₂
  have h₁ : g₁.1 ≐ ⊠⊠(⊤ : α) :=
    thm_godel_fixpoint_equiv_double_reft_top_000004 (α := α) g₁
  have h₂ : g₂.1 ≐ ⊠⊠(⊤ : α) :=
    thm_godel_fixpoint_equiv_double_reft_top_000004 (α := α) g₂
  exact ⟨le_trans h₁.1 h₂.2, le_trans h₂.1 h₁.2⟩
...
theorem main_thm_godel_fixpoint_iff_reft_top_idempotent :
    Nonempty (ACR.GödelFixpoint α) ↔ (⊠(⊤ : α)) ≐ (⊠⊠(⊤ : α)) := by
  constructor
  · simpa using thm_reft_top_equiv_double_reft_top_000008 (α := α)
  · simpa using thm_godel_fixpoint_of_reft_top_equiv_double_000014 (α := α)
