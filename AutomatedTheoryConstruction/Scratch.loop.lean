...
noncomputable def atomicResidualReverseSweep (p : AtomicResidualState) (L : List AtomicResidualState) :
    AtomicResidualState → Bool := by
  classical
  exact
    List.foldr
      (fun q good t =>
        if t = q then
          decide (q = ([# (p.2)], p.2) ∨
            ∃ r : AtomicResidualState, AtomicResidualGraphStep q r ∧ good r = true)
        else
          good t)
      (fun _ => false)
      L
...
theorem thm_foldr_reverse_sweep_accepts_000142 : ∀ p : AtomicResidualState, ∀ R : Finset AtomicResidualState,
  ...
  · intro L
    induction L with
    | nil =>
        intro hnodup hmem hclosed htop hq
        cases hq
    | cons a t ih =>
        intro hnodup hmem hclosed htop hq
