...
/-- A concrete singleton derivation reused as a counterexample witness. -/
theorem singleton_bad_sequent_counterexample : ([#"p"] ⇒ (#"p" ⧸ (#"p" ⧹ #"p"))) := by
...
theorem singleton_atom_shape_fold_branch_absurd
    {s : String} {Δ : List Tp}
    (hΔ : Δ ⇒ # s) :
    ¬ ((# s : Tp) = (# s ⧸ List.foldl Tp.ldiv (# s) Δ) ∨
        (# s : Tp) = List.foldr Tp.rdiv (# s) Δ) := by
  intro hEq
  ...
  | nil =>
      exact hne rfl
  | cons A Γ =>
      rcases hEq with hEq | hEq <;> simp at hEq
...
theorem thm_singleton_atom_derivation_shape_000009_is_false : ¬(∀ {s : String} {A : Tp}, ([# s] ⇒ A) → ∃ Δ : List Tp, Δ ⇒ # s ∧ (A = (# s ⧸ List.foldl Tp.ldiv (# s) Δ) ∨ A = List.foldr Tp.rdiv (# s) Δ)) := by
  ...
  exact singleton_atom_shape_fold_branch_absurd hΔ hEq
